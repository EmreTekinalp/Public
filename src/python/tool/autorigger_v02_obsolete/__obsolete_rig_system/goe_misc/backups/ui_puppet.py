...
path = os.path.join(os.path.dirname(__file__),'ui', 'puppetSettingsDialog.ui')
assert os.path.exists(path), 'Path does not exist!'

form_class, base_class = utils.load_ui_type(path)


class PuppetSettingsUI(base_class, form_class):
    def __init__(self, buttonName='Create'):
        super(PuppetSettingsUI, self).__init__()
        self.setupUi(self)
        self.setGeometry(600,300,500,427)
    ...
    def __create_menu_bar(self):
        ...
        help_menu.addAction('Help on Puppet Settings')
...
#             self.mirror_chb.setDisabled(True)
        ...
        #--- disable line edits
        self.buffer_led.setDisabled(disable)
    ...
    def __check_line_edit(self):
        if self.buffer_led.isModified():
            self.amount_sli.setValue(int(self.buffer_led.text()))
    ...
    def __setup_line_edit(self):
        ...
        self.buffer_led.setValidator(validatora)
    ...
    def __check_slider(self):
        if self.amount_sli.valueChanged:
            self.buffer_led.setText(str(self.amount_sli.value()))
...
#                      int(maxis), int(asym)]
        super(PuppetSettingsUI, self).accept()
        self.close()
        self.data = ['print this shit']
        return self.data
    #END get_data()
    ...
    def apply_data(self, data):
        ...
        #--- amount
        self.buffer_led.setText(str(data[4]))
...
#         self.buffer_led.editingFinished.connect(self.__check_line_edit)
