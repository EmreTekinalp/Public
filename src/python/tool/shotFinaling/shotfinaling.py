...
class ShotFinal():
        ...
        if sel:
            ...
            if cmds.listRelatives(transform, allDescendents = True, type = 'mesh'):
                ...
                if cmds.nodeType(mesh) == 'mesh':
                    #--- store the mesh
                    if 'FINAL' in transform:
                        self.mesh = transform.split('FINAL')[0]
                    else:
                        self.mesh = transform
                    #--- get current frame
    ...
    def __setup_scene(self):
        ...
        if cmds.objExists(mesh + '_BSP_GRP'):
            self.mesh_bsp_grp = mesh + '_BSP_GRP'
        else:
            self.mesh_bsp_grp = cmds.createNode('transform',
                                                name = mesh + '_BSP_GRP',
                                                parent = self.blendshapes)
        #--- check if the sculpt group exists
        if cmds.objExists(mesh + '_SCULPT_GRP'):
            self.sculpt_grp = mesh + '_SCULPT_GRP'
        else:
            self.sculpt_grp = cmds.createNode('transform',
                                              name = mesh + '_SCULPT_GRP',
                                              parent = self.mesh_bsp_grp)
        #--- check if the negative group exists
        if cmds.objExists(mesh + '_NEGATIVE_GRP'):
            self.negative_grp = mesh + '_NEGATIVE_GRP'
        else:
            self.negative_grp = cmds.createNode('transform',
                                                name = mesh + '_NEGATIVE_GRP',
                                                parent = self.mesh_bsp_grp)
        #--- check if the result group exists
        if cmds.objExists(mesh + '_RESULT_GRP'):
            self.result_grp = mesh + '_RESULT_GRP'
        else:
            self.result_grp = cmds.createNode('transform',
                                              name = mesh + '_RESULT_GRP',
                                              parent = self.mesh_bsp_grp)
    #end def __setup_scene()

    def __setup_final_meshes(self):
        #--- this method setups the final meshes
        if not cmds.objExists(self.mesh + 'FINAL'):
            if not cmds.listRelatives(self.mesh, parent = True):
                if not cmds.objExists('SHOTFINALING_GEO_GRP'):
                    sf_grp = cmds.createNode('transform', name = 'SHOTFINALING_GEO_GRP')
                    cmds.parent(self.mesh, sf_grp)
                else:
                    sf_grp = 'SHOTFINALING_GEO_GRP'
                    cmds.parent(self.mesh, sf_grp)
            abc_grp = cmds.listRelatives(self.mesh, parent = True)[0]
            if not abc_grp == 'SF_FINAL_MESHES':
                abc_meshes = cmds.listRelatives(abc_grp,
                                                children = True)
                #--- parent the duplicated abc meshes to SF_FINAL_MESHES
                for dup in abc_meshes:
                    final_mesh = cmds.duplicate(dup)
                    print final_mesh, dup
                    cmds.parent(final_mesh, self.sf_final_meshes)
                    final_name = cmds.rename(final_mesh, dup + 'FINAL')

                    #--- setup blendShape deformer
                    if cmds.objExists(dup + '_BSP'):
                        self.final_bsp = dup + '_BSP'
                        cmds.setAttr(self.final_bsp + '.' + dup, 1)
                        cmds.setAttr(dub + '.v', 0)
                    else:
                        self.final_bsp = cmds.blendShape(dup,
                                                         final_name,
                                                         name = (dup + '_BSP'))[0]
                        if ':' in dup:
                            dup_attr = dup.split(':')[-1]
                        else:
                            dup_attr = dup
                        cmds.setAttr(self.final_bsp + '.' + dup_attr, 1)
                        cmds.setAttr(dup + '.v', 0)
                    #--- hide the alembic group
                    if cmds.listAttr(dup + '.v', locked = True):
                        cmds.setAttr(dup + '.v', lock = False, keyable = True)
                    cmds.setAttr(dup + '.v', 0)

        if not 'FINAL' in self.mesh:
        ...
        #--- setup blendShape deformer
        if 'FINAL' in self.mesh:
            alembic = self.mesh.split('FINAL')[0]
        else:
            alembic = self.mesh
        if cmds.objExists(alembic + '_BSP'):
            self.final_bsp = alembic + '_BSP'
            if ':' in alembic:
                alembic_attr = alembic.split(':')[-1]
            else:
                alembic_attr = alembic
            cmds.setAttr(self.final_bsp + '.' + alembic_attr, 1)
            cmds.setAttr(alembic + '.v', 0)
        else:
            self.final_bsp = cmds.blendShape(alembic,
                                             self.final_mesh,
                                             name = (alembic + '_BSP'))[0]
            if ':' in alembic:
                alembic_attr = alembic.split(':')[-1]
    ...
    def __setup_blendshapes(self):
        ...
        if self.final_bsp:
            if cmds.objExists(self.final_bsp):
                ...
                plug_list = []
                for i in world_mesh:
                    cnt_list = cmds.listConnections(i,plugs = True,destination = True)
                    for cnt in cnt_list:
...
class RemoveShotFinal():
    ...
    @DONE: delete all the empty transform nodes
    '''
                ...
                if 'Sculpt' in i:
                    ...
                    if cmds.objExists(result_mesh):
                        ...
                        for cnt in connections:
                            if cnt == i.split("FINALSculpt")[0] + '_BSP':
                                bshape.append(cnt)
                        ...
                        if cmds.listRelatives(tool_main, children = True):
                            ...
                            if not tool_child:
                                ...
                                if not bsp_child:
                                    cmds.delete(bsp_grp)
                                if not cmds.listRelatives(i.split("FINALSculpt")[0] +
                                                          "_SCULPT_GRP",
                                                          children = True):
                                    cmds.delete(i.split("FINALSculpt")[0] + '_SCULPT_GRP')
                                    cmds.delete(i.split("FINALSculpt")[0] + '_NEGATIVE_GRP')
                                    cmds.delete(i.split("FINALSculpt")[0] + '_RESULT_GRP')
                                    cmds.delete(i.split("FINALSculpt")[0] + '_BSP_GRP')
