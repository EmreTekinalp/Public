...
#            cmds.currentTime(pre)
            cmds.setKeyframe(self.sf_mesh_ctl + '.blendValue', time=pre, value=0)
            #--- set key one frame after currentFrame
            post = self.current_frame + offset
#            cmds.currentTime(post)
            cmds.setKeyframe(self.sf_mesh_ctl + '.blendValue',time=post, value=0)
    ...
    def __sf_info_node(self):
        #--- this method creates a node which stores all necessary information
        if cmds.objExists('SF_INFO_NODE_' + self.character):
            self.sf_node = 'SF_INFO_NODE_' + self.character
            cmds.lockNode(self.sf_node, lock=False)
        else:
            self.sf_node = cmds.createNode('transform', name = 'SF_INFO_NODE_' + self.character)
        #--- lock all transform attributes and the node
        for axis in 'xyz':
            cmds.setAttr(self.sf_node + '.t' + axis, lock=True, keyable=False)
            cmds.setAttr(self.sf_node + '.r' + axis, lock=True, keyable=False)
            cmds.setAttr(self.sf_node + '.s' + axis, lock=True, keyable=False)
        cmds.setAttr(self.sf_node + '.v', lock=True, keyable=False)

        #--- sculpt control and mesh attribute
        if not cmds.objExists(self.sf_node + '.' + self.mesh + 'SculptCTL' + str(self.target_index)):
            mesh = self.mesh
            if '|' in self.mesh:
                mesh = self.mesh.split('|')[-1]
            cmds.addAttr(self.sf_node,
                         longName=(mesh + 'SculptCTL' + str(self.target_index)),
                         shortName=(mesh + 'SculptCTL' + str(self.target_index)),
                         attributeType='message')
            cmds.connectAttr(self.sf_mesh_ctl + '.message',
                             self.sf_node + '.' + mesh + 'SculptCTL' + str(self.target_index))
        else:
            mesh = self.mesh
            if '|' in self.mesh:
                mesh = self.mesh.split('|')[-1]           
            #--- check if value exists, else give value
            attr = cmds.listConnections(self.sf_node + '.' + mesh + 'SculptCTL' + str(self.target_index))
            if attr:
                if not attr == self.sf_mesh_ctl:
                    cmds.connectAttr(self.sf_mesh_ctl + '.message',
                                     self.sf_node + '.' + mesh + 'SculptCTL' + str(self.target_index))
            else:
                cmds.connectAttr(self.sf_mesh_ctl + '.message',
                                 self.sf_node + '.' + mesh + 'SculptCTL' + str(self.target_index))
        cmds.lockNode(self.sf_node, lock=True)
        character = self.character
    #END def __sf_info_node()
...
class RemoveShotFinal():
        ...
        if sel:
            ...
            if '_SCULPT' in sel[0]:
                ...
                self.mesh_grp = cmds.listRelatives(self.mesh,
                                                   parent = True,
                                                   fullPath = True)[0]
                #--- get the shotfinal_info_node
                if cmds.objExists('SF_INFO_NODE_' + self.character):
                    self.sf_info_node = 'SF_INFO_NODE_' + self.character
    ...
    def __remove_sf_info(self):
        #--- this method removes the sf info node message or its attributes
        attributes = cmds.listAttr(self.sf_info_node, userDefined=True)
        if not attributes:
            cmds.delete(self.sf_info_node)
        for attr in attributes:
            attr_type = cmds.attributeQuery(attr, node=self.sf_info_node, 
                                            attributeType=True)
            if attr_type == 'message':
                connections = cmds.listConnections(self.sf_info_node + '.' + attr, 
                                                   plugs=True)
                if not connections:
                    cmds.lockNode(self.sf_info_node, lock=False)
                    cmds.deleteAttr(self.sf_info_node, attribute=attr)
                    cmds.lockNode(self.sf_info_node, lock=True)
                    print ('Removed message connection of ' 
                           + self.sf_info_node + '.' + attr)
        attributes = cmds.listAttr(self.sf_info_node, userDefined=True)
        if not attributes:
            cmds.lockNode(self.sf_info_node, lock=False)
            cmds.delete(self.sf_info_node)
    #END def __remove_sf_info()
