"""
@package: brick.draft
@brief: Base implementations of the guide base class
...
from utility import node, storage
...
class Blueprint(interface.RiggingInterface):
    ...
    def _blueprint_group(self):
        ...
        if not (pm.objExists('%s_%sBlueprint_GRP' % (self.side, self.name))):
            grp = node.group('%sBlueprint' % self.name)
